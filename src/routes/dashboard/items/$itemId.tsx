...
import { getItembyId, saveSummarAndGenerateTagsFn } from '@/data/items'
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
export const Route = createFileRoute('/dashboard/items/$itemId')({
  ...
  head: ({ loaderData }) => ({
    meta: [
      ...
      {
        property: 'og:image',
        content: loaderData?.ogImage ?? 'dsösşldgösdil',
      },
      { name: 'twitter:title', contet: loaderData?.title ?? 'Item Details' },
    ],
  }),
})
...
function RouteComponent() {
  ...
  const { completion, complete, isLoading } = useCompletion({
    ...
    onFinish: async (_prompt, completionText) => {
      await saveSummarAndGenerateTagsFn({
        data: {
          id: data.id,
    ...
    <div className="mx-auto max-w-3xl w-full space-y-6">
      ...
      <div className="relative aspect-video w-full overflow-hidden rounded-lg bg-muted">
        <img
          ...
          src={
            data.ogImage ??
            'https://images.unsplash.com/photo-1635776062043-223faf322554?q=80&w=1932&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'
          }
